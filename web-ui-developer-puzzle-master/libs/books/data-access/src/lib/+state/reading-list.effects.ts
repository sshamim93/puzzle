...
export class ReadingListEffects implements OnInitEffects {
  ...
  updateReadingItem$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ReadingListActions.updateReadingItem),
      optimisticUpdate({
        run: ({ item }) => {
          return this.http.put(`/api/reading-list/${item.bookId}/finished`,item).pipe(
            map(() =>
              ReadingListActions.confirmUpdateReadingItem({
                item
              })
            )
          );
        ...
        undoAction: ({ item }) => {
          return ReadingListActions.failedUpdateReadingItem({
            item
          });
        }
      })
